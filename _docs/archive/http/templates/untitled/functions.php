...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    function artxPost($data)
    {
    	...
    	artxFragmentBegin(str_replace('class="box post">', 'class="box post' . $classes . '">', "<div class=\"box post\">\r\n    <div class=\"box-body post-body\">\r\n<div class=\"post-inner\">\r\n"));
...
<?php
        ...
        echo str_replace('class="box vmenublock">', 'class="box vmenublock' . $classes . '">', ob_get_clean());
        if ($hasCaption):
...
<?php
    ...
    function artxModules(&$document, $position, $style = null)
    {
        ...
        function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
        {
            ...
            foreach (artxFindTags($content, $filter) as $tag) {
                if (is_string($tag))
                    ...
                    $button = str_replace('"', $delimeter, '<span class="button-wrapper">'
                        . '<span class="button-l"> </span><span class="button-r"> </span>')
        ...
        {
            return '<span class="button-wrapper"><span class="button-l"> </span><span class="button-r"> </span>'
                . '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
                . 'button" href="' . $data['link'] . '">' . $data['content'] . '</a></span>';
        }
...
function artxPostprocessBlockContent($content)
    ...
    function artxPostprocessContent($content)
    {
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!button')),
            'button' => array('class' => array('button', '!button'))));
        return $content;
    }
