...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="content-layout">';
        $result .= '<div class="content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="layout-cell' . ('block' == $style ? ' layout-sidebar-bg' : '')
     ...
     * Then it wraps content of the buffer with post.
     */
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'main').addClass('postcontent');
            }
        });
    }
});
